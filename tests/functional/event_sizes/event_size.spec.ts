...
import { EventSizeFactory, RoleFactory, UserFactory } from 'Database/factories'
...
test.group('Event Size', (group) => {
  ...
  test('store - it should create an event size', async ({ client, assert }) => {
    const eventPayload = { description: 'test' }
    const response = await client
      .post(`${BASE_URL}/event-sizes`)
      .header('Authorization', `Bearer ${tokenBasic}`)
      .json(eventPayload)
    ...
    assert.exists(body.data, 'event size undefined')
  ...
  test('store - it should return 422 when description is alreay in use', async ({
    ...
    const { description } = await EventSizeFactory.create()
    const response = await client
      .post(`${BASE_URL}/event-sizes`)
      .header('Authorization', `Bearer ${tokenBasic}`)
      .json({
  ...
  test('update - it should update an event size', async ({ client, assert }) => {
    const { id } = await EventSizeFactory.create()
    const description = 'testing'
    const response = await client
      .put(`${BASE_URL}/event-sizes/${id}`)
      .header('Authorization', `Bearer ${tokenBasic}`)
      .json({ description })
  ...
  test('index - it should return list with data', async ({ client, assert }) => {
    const { id, description } = await EventSizeFactory.create()

    const response = await client.get(`${BASE_URL}/event-sizes`)
  ...
  test('show - it should return list with data', async ({ client, assert }) => {
    const { id, description } = await EventSizeFactory.create()
    const response = await client.get(`${BASE_URL}/event-sizes/${id}`)
  ...
  test('destroy - it should 204 when delete the data', async ({ client, assert }) => {
    ...
    const responseOne = await client
      .delete(`${BASE_URL}/event-sizes/${id}`)
      .header('Authorization', `Bearer ${tokenBasic}`)

    responseOne.assertStatus(204)

    const responseTwo = await client.get(`${BASE_URL}/event-sizes/${id}`)
