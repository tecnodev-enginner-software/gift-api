...
import { ModalityFactory, RoleFactory, UserFactory } from 'Database/factories'
...
test.group('Modality', (group) => {
  ...
  test('store - it should create an modality', async ({ client, assert }) => {
    const modalityPayload = { description: 'test' }
    const response = await client
      .post(`${BASE_URL}/modalities`)
      .header('Authorization', `Bearer ${tokenBasic}`)
      .json(modalityPayload)

    response.assertStatus(201)

    const body = response.body()

    assert.exists(body.data, 'modality undefined')
    assert.exists(body.data.id, 'id undefined')
    assert.equal(body.data.description, modalityPayload.description)
  })
  ...
  test('store - should return 422 when the description has no minimum length', async ({
    ...
    const modalityPayload = { description: 'te' }
    const response = await client
      .post(`${BASE_URL}/modalities`)
      .header('Authorization', `Bearer ${tokenBasic}`)
      .json(modalityPayload)
  ...
  test('store - should return 422 when the description does not respect the regex', async ({
    ...
    const modalityPayload = { description: '#0[' }
    const response = await client
      .post(`${BASE_URL}/modalities`)
      .header('Authorization', `Bearer ${tokenBasic}`)
      .json(modalityPayload)
  ...
  test('store - it should return 422 when description is alreay in use', async ({
    ...
    const { description } = await ModalityFactory.create()
    const response = await client
      .post(`${BASE_URL}/modalities`)
      .header('Authorization', `Bearer ${tokenBasic}`)
      .json({
  ...
  test('update - it should update an modality', async ({ client, assert }) => {
    const { id } = await ModalityFactory.create()
    const description = 'testing'
    const response = await client
      .put(`${BASE_URL}/modalities/${id}`)
      .header('Authorization', `Bearer ${tokenBasic}`)
      .json({ description })
  ...
  test('index - it should return list with data', async ({ client, assert }) => {
    const { id, description } = await ModalityFactory.create()

    const response = await client.get(`${BASE_URL}/modalities`)
  ...
  test('show - it should return list with data', async ({ client, assert }) => {
    const { id, description } = await ModalityFactory.create()
    const response = await client.get(`${BASE_URL}/modalities/${id}`)
  ...
  test('destroy - it should 204 when delete the data', async ({ client, assert }) => {
    ...
    const responseOne = await client
      .delete(`${BASE_URL}/modalities/${id}`)
      .header('Authorization', `Bearer ${tokenBasic}`)

    responseOne.assertStatus(204)

    const responseTwo = await client.get(`${BASE_URL}/modalities/${id}`)
